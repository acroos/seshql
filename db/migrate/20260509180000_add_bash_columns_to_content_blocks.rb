class AddBashColumnsToContentBlocks < ActiveRecord::Migration[8.1]
  def up
    execute <<~SQL
    ...
    execute <<~SQL
      ALTER TABLE content_blocks
        ...
        GENERATED ALWAYS AS (
          CASE WHEN tool_name = 'Bash' THEN tool_input->>'command' END
        ) STORED;
    SQL
  ...
  def down
    ...
    execute "DROP FUNCTION IF EXISTS bash_programs(text);"
  end
end
