# Claude API token pricing, used to turn raw usage counts into a dollar figure.
#
...
module Pricing
  ...
  # The priced categories a turn's tokens fall into, cheapest-explaining first.
  # Labels are what the UI shows; the ordering is the order they're displayed in.
  COMPONENTS = {
  ...
  # Splits one assistant turn's `usage` hash into priced components.
  def self.components_for_usage(model, usage)
    ...
    # Older transcripts record only the total, with no 5m/1h split. Bill the
    # remainder at the 5-minute rate, which is what Claude Code writes by default.
  ...
  def self.web_search_requests(usage)
    (usage.presence || {}).dig("server_tool_use", "web_search_requests").to_i
  end

  # Cost in USD for one assistant turn. Returns nil for models we have no rate
  # for (including Claude Code's synthetic error messages).
  def self.cost_for_usage(model, usage)
    components = components_for_usage(model, usage)
    return nil if components.any? { |c| c.cost_usd.nil? }
    components.sum(&:cost_usd) + (web_search_requests(usage) * WEB_SEARCH_USD)
  end

  def self.rates_for(model, speed: nil)
    id = normalize(model)
